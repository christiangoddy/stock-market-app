...
import { formatArticle, formatChangePercent, formatMarketCapValue, formatPrice, getDateRange, validateArticle } from '../utils';
...
//  fetches data to be displayed in the watchlist table
...
// onl used to get the watchlist and then 
...
export const getUserWatchlist = async () => {
  try {
    ...
    const watchlist = await Watchlist.find({ userId: session.user.id })
      .sort({ addedAt: -1 })   // desc i.e newest added first
      .lean();
